...
function LevelCard() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 text-black dark:bg-gray-900 dark:text-white">
      <h2 className="text-3xl font-bold text-center mb-6">Level {level} Cards</h2>

      {cards.length === 0 ? (
        <p className="text-center text-lg text-gray-500">No cards available for this level.</p>
      ) : (
        <div className="grid grid-cols-3 gap-6">
          {cards.map((card) => (
            <div key={card._id} className="relative bg-white px-6 pt-6 pb-3 rounded-lg shadow-gray-600 shadow-lg min-h-50">
              {/* Level Badge */}
              ...
              <p className="text-gray-900 my-2 mb-7 text-lg">Answer: {card.answer}</p>
